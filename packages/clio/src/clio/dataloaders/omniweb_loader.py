...
https://omniweb.gsfc.nasa.gov/form/dx1.html
Specific order is mandatory. Columns in response ordered by: id => asc
So we should use same order in dictionary to handle this
"""
DEFAULT_VARS = {
    "bx": {"id": 12, "title": "Bx, GSE/GSM, nT"},
    "by": {"id": 15, "title": "BY, nT (GSM)"},
    "bz": {"id": 16, "title": "BZ, nT (GSM)"},
    "t": {"id": 22, "title": "SW Plasma Temperature, K"},
    "n": {"id": 23, "title": "SW Proton Density, N/cm^3"},
    "v": {"id": 24, "title": "SW Plasma Speed, km/s"},
    "kp": {"id": 38, "title": "Kp index"},
    "dst": {"id": 40, "title": "Dst-index, nT"},
    "ap": {"id": 49, "title": "ap_index, nT"},
    "f10_7": {"id": 50, "title": "f10.7_index"},
}

from common.data.omni import OMNI_FILL_VALUES, clean_omni_values


class OMNIWeb_Loader:
    def load(start_date: datetime, end_date: datetime) -> pd.DataFrame:
        start_date_omni = start_date.strftime("%Y%m%d")
        end_date_omni = end_date.strftime("%Y%m%d")
        ...
        payload = {
            ...
            "scale": "Linear",
            "spacecraft": "omni2",
            "start_date": start_date_omni,
            "end_date": end_date_omni,
            "table": 0,
        }
    ...
    def _parse_omni_text(text: str) -> pd.DataFrame:
        ...
        return df[["issue_time", "bx", "by", "bz", "v", "n", "t", "kp", "dst", "ap", "f10_7"]]
