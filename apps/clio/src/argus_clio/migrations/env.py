"""Migrations are scoped to clio; provision/adopt legacy storage first."""
from alembic import context
from sqlalchemy import create_engine, pool, text
from argus_clio.db.base import Base
from argus_clio.db import models  # noqa: F401
...
if context.is_offline_mode():
    ...
    with context.begin_transaction():
        context.execute('SET search_path TO clio, pg_catalog, pg_temp')
        context.run_migrations()
        context.execute('REVOKE ALL ON clio.alembic_version FROM argus_clio')
else:
    ...
    with engine.connect() as connection:
        ...
        with context.begin_transaction():
            context.run_migrations()
            if connection.scalar(text("SELECT to_regclass('clio.alembic_version')")):
                connection.execute(text('REVOKE ALL ON clio.alembic_version FROM argus_clio'))
    engine.dispose()
