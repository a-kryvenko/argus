...
def wind_forecast():
    config_path = "" # path to surya config
    checkpoint_path = "" #path to model checkpoints
    save_embedding_path = "" #path to save embeddings
    gpu = False
    ...
    # Get dataloader
    dataset, dataloader = _get_dataloader(config, scalers, config["dtype"], 3)
    ...
    with torch.amp.autocast(device_type=device, dtype=config["dtype"]):
        ...
        embedding = model(batch, return_embedding=True) # [1, D]
        embedding = embedding.squeeze(0).detach().cpu().numpy()
        timestamps_input_str = np.datetime_as_string(timestamps_input, unit='m')[0][0]
        np.savez(
            save_embedding_path,
            timestamps=np.array(timestamps_input_str, dtype=str),
...
def _get_dataloader(config, scalers, data_type="test", num_samples=3):
    """
