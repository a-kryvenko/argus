import pandas as pd
from common.schemas.forecast import Forecast, WindForecastPoint
...
def wind_forecast_to_dataframe(forecast: Forecast) -> pd.DataFrame:
    return pd.DataFrame([f.model_dump() for f in forecast.points])

def wind_forecast_from_dataframe(df: pd.DataFrame) -> Forecast:
    points = []
