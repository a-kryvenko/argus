-- Read-only snapshot. Full row scans, run off-peak on large databases.
...
WHERE relname IN ('solar_wind_observation', 'geomagnetic_observation', 'observation_source_status');
SELECT kind, spacecraft, count(*) AS rows, count(*) FILTER (WHERE active) AS active_rows,
       ...
       round(avg(pg_column_size(values))) AS mean_values_bytes
FROM solar_wind_observation s GROUP BY kind, spacecraft ORDER BY kind, spacecraft;
...
FROM geomagnetic_observation g GROUP BY metric ORDER BY metric;
