...
from argus_clio.services.observations.density_history import download_solar_index_history
...
async def backfill(session, start, end):
    ...
    await upsert_measurements(session, measurements, track_receipt=False, replace_existing=False)
    stored = await load_measurements(session, since=start, until=end)
